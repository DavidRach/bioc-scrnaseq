...
You can see we ended up with `r length(levels(sce$clusters))` clusters of varying sizes.
...
-   The `clusterGraph.se()` function from the `r Biocpkg("scrapper")` package 
...
-   The `scoreMarkers.se()` function from the `r Biocpkg("scrapper")` package 
