...
in the `r Biocpkg("scrapper")` package, using the `sample` column as the batch variable.
...
-   Computational removal of batch-to-batch variation with the `correctMnn.se()`
    function from the `r Biocpkg("scrapper")` package allows us to combine data
...
-   The `aggregateAcrossCells.se()` function from the `r Biocpkg("scrapper")` package
